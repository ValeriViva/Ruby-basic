...
class Railway
  ...
  def action
    action = { a: method(:add_station), b: method(:delete_station), c: method(:add_route), d: method(:add_carriage),
               e: method(:delete_carriage), f: method(:move_to_next_station), g: method(:move_to_previous_station),
               h: method(:take_capacity) }
    loop do
  ...
  def delete_carriage
    ...
    train = @trains[gets.chomp.to_i]
    train.stop
    return unless train.carriages.size.positive?
  ...
  def take_capacity
    ...
    if carriage.available_capacity.positive?
